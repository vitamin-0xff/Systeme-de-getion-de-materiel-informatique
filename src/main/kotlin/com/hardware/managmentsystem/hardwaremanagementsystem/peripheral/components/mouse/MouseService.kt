...
@Service
class MouseService(
    private val repo: MouseRepository,
    private val marques: MarqueRepository,
    private val providers: ProviderRepository
) {
    @Transactional
    fun create(req: MouseRequest): MouseResponse {
    ...
    fun list(pageable: Pageable): Page<MouseResponse> = repo.findAll(pageable).map { it.toResponse() }

    @Transactional(readOnly = true)
    fun get(id: UUID) = repo.findById(id).orElseThrow { RuntimeException("Mouse not found: $id") }.toResponse()

    @Transactional
    fun update(id: UUID, u: MouseUpdate): MouseResponse {
        val entity = repo.findById(id).orElseThrow { RuntimeException("Mouse not found: $id") }
        ...
        entity.applyUpdate(u, provider)
        return repo.save(entity).toResponse()
    }

    @Transactional
    fun delete(id: UUID) {
        if (!repo.existsById(id)) throw RuntimeException("Mouse not found: $id")
        repo.deleteById(id)
    }
}
